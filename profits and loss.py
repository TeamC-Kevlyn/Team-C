...
for day in summary_list:
    ...
    for item in profitsandloss:
        if day == item[0]:
            sales += (int(item[1]))
            trading_profit += (int(item[2]))
            operating_expense += (int(item[3]))
            net_profit += (int(item[4]))
    summary_list.append(f"{day},{sales}, {trading_profit}, {operating_expense},{net_profit}")

print(summary_list)

with open('profits and loss.txt', 'w') as file:
    file.write('Records of profits and loss\n')
    file.write('===============\n')
    file.write(" Day, sales, trading profit, operating expense, netprofit\n")
